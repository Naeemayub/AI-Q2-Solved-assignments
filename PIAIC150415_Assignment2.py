...
def function7():
    ...
    return np.array([0]*4, dtype="int64")
...
def function12():
    ...
    arr = np.array([1,2,3])
    arr2 = np.dstack((arr,arr,arr)).flatten()
    arr3 = np.vstack((arr,arr,arr)).flatten()
...
def function18():
    ...
    x = np.size(np.intersect1d(students,piaic))
    return x
